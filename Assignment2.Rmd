...
```
The test code for share.fn and delta.fn. The result is the distribution of T(delta) - delta, where T is a contraction mapping. It's distribution is quite close to 0, thus the delta function is valid.
```{r Problem 2 Test, echo=FALSE,warning=FALSE,eval=TRUE}
#Test code
...
```
Then we need to check the derivative function is correct. The result printed is distribution of difference between numerical derivative and analytical derivate I wrote. Observing that the derivative is quite close to 0, then the function is likely to be correctly written.
...
step.a <- nloptr(x0=x0 ,eval_f = obj.func, lb = x0 * 0.1 , opts=list(algorithm="NLOPT_LN_BOBYQA",tol=1e-2))
sol <- step.a$solution
...
```
I tried to use weight matrix as the inverse of outer product of instruments. However, this result seems to be even worse. The scale of alpha is far away from the estimation.
...
```
I think the result is a bit different than the paper's table VI. In the paper, the elasticity with own price is lowest for BW735i. However, this is against intuition, because among all the models, BW735i has the highest price. It's elasticity should have the largest magnitude.
                    ...
                    market.id,firm.id, var)
{
  ...
  for (t in unique(market.id)) {
    ...
    B.B <- solve( t(M.B %*% B.dr) %*%  (M.B %*% B.dr))
    
    list(alpha.sigma.var = alpha.sigma.var <-   t(M.B %*% B.dr) %*% var %*% (M.B %*% B.dr), B.dr  = B.dr)
...
sd <- matrix(c(sqrt(diag(alpha.sigma.var$alpha.sigma.var)),sqrt(diag(var))),nc=1) #Calculate standard error
